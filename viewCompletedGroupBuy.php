                ...
                <table id="results-section" class="responsive table table-striped table-bordered" style="width:100%;margin-bottom:0; ">
                    ...
                    if (is_array($orderProduct)) {
                        ...
                        foreach ($orderProduct as $i => $value) {
                            ...
                            if ($product->getType() == 'grain' || $product->getType() == 'hops') {
                                $foodTotal = $foodTotal + ($price*$order->getAmount());
                            } else {
                                $otherTotal = $otherTotal + ($price*$order->getAmount());
                            }
                            print '<tr><td>' . $order -> getAmount() . '</td>';
                            $name = "";
                    ...
                    if (is_array($orderSplit)) {
                        ...
                        foreach ($orderSplit as $y => $value) {
                            ...
                            $totalPounds = $totalPounds+($pounds * $order -> getAmount());
                            if ($adminUpdate) {
