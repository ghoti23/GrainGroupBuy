...
class dao
{
    ...
    public function addUser($user)
    {
        try {
            ...
            if ($count == 0) {
                $sql = "INSERT INTO user (email, username, password, firstName, lastName, zipCode) VALUES (?,?,?,?,?,?)";
                $pdo = $this->pdoObject;
                $sth = $pdo->prepare($sql);
                $sth->execute(array($user->getEmail(), $user->getUsername(), $user->getPassword(), $user->getFirstName(), $user->getLastName(), $user->getZipCode()));
                return true;
            } else {
                return false;
            }
